    ...
    <span
      className={
        ok
          ...
          : "inline-flex items-center gap-1 rounded-full border border-border-strong/30 bg-muted-foreground/10 px-2 py-1 text-xs text-fg-subtle"
      }
    >
      ...
      <SectionBody width="narrow" className="space-y-4">
        <div className="rounded-xl border border-border/70 bg-card p-4" role="region" aria-label="Quick start setup">
          ...
          {isExtensionUnavailable && (
            <div className="mb-3 flex items-start gap-2 rounded-lg border border-info-border bg-info-bg px-3 py-2 text-xs text-info-fg">
            ...
            {setupSteps.map((step, idx) => (
              ...
              >
                <p className="text-xs font-medium text-foreground">
                  {idx + 1}. {step.title}
                  <span className={step.done ? "ml-2 text-success-fg" : "ml-2 text-fg-subtle"}>
            ...
            <div className="space-y-3">
              {primaryTab && (
                <div className="rounded-lg border border-success-border bg-success-bg px-3 py-2">
                  <p className="text-xs font-semibold text-success-fg">Primary connected tab</p>
                  <p className="mt-1 text-sm text-foreground">{tabTitle(primaryTab)}</p>
        ...
        {guidance.length > 0 && (
          <div className="rounded-xl border border-warning-border bg-warning-bg p-4">
            <div className="mb-2 inline-flex items-center gap-1 text-sm font-medium text-warning-fg">
              <Sparkles className="h-4 w-4" />
              What to do next
            </div>
            <ul className="space-y-1 text-xs text-warning-fg">
              {guidance.map((note, i) => (
        ...
        {friendlyError && (
          <div className="rounded-md border border-danger-border bg-danger-bg p-3 text-xs text-danger-fg" role="alert">
            {friendlyError}
            {showAdvanced && error && friendlyError !== error && (
              <p className="mt-2 text-[11px] text-danger-fg">Raw error: {error}</p>
            )}
          </div>
        )}

        {notice && (
          <div className="rounded-md border border-success-border bg-success-bg p-3 text-xs text-success-fg" role="status">
            {notice}
          </div>
        )}
        ...
        {showAdvanced && (
          <div className="rounded-xl border border-border/70 bg-card p-4">
            ...
            {(snapshot?.errors.status || snapshot?.errors.tabs || snapshot?.extension.error) && (
              <div className="mt-3 rounded-md border border-warning-border bg-warning-bg p-2 text-xs text-warning-fg">
